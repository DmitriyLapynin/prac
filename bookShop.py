...
class BookShop:
    ...
    def __init__(self, books_file: str):
        self.book_list = self.extract_books(books_file, self.start_margin, self.margin_new, self.start_rating)
        ...
        self.orders = []
        self.sold = [deepcopy(book) for book in self.book_list]
        for book in self.sold: book.copies_num = 0
        for i in range(len(self.sold)):
            print(self.sold[i], self.book_list[i])
        self.applications = []
        self.income = 0

    def extract_books(self, books_file: str, margin, margin_new, rating) -> List[Book]:
        ...
        with open(books_file, 'r', encoding='cp1251') as in_file:
            ...
            for row in csv_reader:
                ...
                name, author = row[:2]
                if row[-3] >= 2022:
                    book = Book(name, author, row[2:], margin_new, rating, self.start_copies)
                else:
                    book = Book(name, author, row[2:], margin, rating, self.start_copies)
    ...
    def try_to_execute_orders(self, day: int) -> List[PublishingOrder]:
        ...
        for order in filter(lambda o: not o.status.is_done(), self.orders):
            ...
            for book in filter(lambda b: b.copies_num != 0, order.book_list):
                ...
                find_book = self.lookup(book)
                need_copies = self.remove(find_book, book.copies_num)
        ...
        for book, copies in self.check_min_copies():
